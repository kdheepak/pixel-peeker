...
impl eframe::App for PixelPickerApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            // Find the monitor and capture image only once
            let mut monitor_image = None;
                ...
                for monitor in monitors {
                        ...
                        && y < mon_y + mon_height as i32
                    {
                        if let Ok(image) = monitor.capture_image() {
                            monitor_image = Some((monitor, image));
                        }
                        break;
                    }
                }
            ...
            // Helper closures to use the captured image
            let get_color_at = |screen_x: i32, screen_y: i32| -> Option<Color32> {
                if let Some((ref monitor, ref image)) = monitor_image {
            ...
            let get_preview_at =
                |screen_x: i32, screen_y: i32, size: u32| -> Option<(Vec<u8>, u32, u32)> {
                    if let Some((ref monitor, ref image)) = monitor_image {
                        let mon_x = monitor.x().ok()?;
                        let mon_y = monitor.y().ok()?;
                        let mon_width = monitor.width().ok()?;
            ...
            // Handle freeze/unfreeze logic (moved after closures)
            if just_pressed {
